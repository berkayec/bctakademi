        ...
        <div className="flex flex-col gap-12">
          ...
          <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground font-medium">
                Tamamladığınız eğitim modülleri için hak kazandığınız belgeler.
              </p>
            </div>
          </header>
          ...
          {earnedCertificates.length > 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
              <div className="lg:col-span-2 space-y-12">
                {earnedCertificates.map((cert) => (
                  <motion.div
                    ...
                    viewport={{ once: true }}
                  >
                    {/* SERTİFİKA KARTI (Görsel belge olduğu için beyaz/açık gri tonu korundu) */}
                    <Card className="overflow-hidden border-none shadow-2xl bg-white dark:bg-slate-50 rounded-[2.5rem] relative">
                      <div className="absolute top-0 right-0 w-64 h-64 bg-teal-500/5 rounded-full -mr-20 -mt-20 blur-3xl" />
                      <CardContent className="p-8 md:p-16 border-[12px] border-double border-slate-100 m-3 rounded-[2rem]">
                    ...
                    {/* Kart altı butonlar: text-slate-400 -> text-muted-foreground | hover:text-white -> hover:text-foreground */}
                    <div className="mt-6 flex flex-wrap gap-4 justify-center md:justify-end px-4">
                      <Button variant="ghost" className="text-muted-foreground hover:text-foreground font-bold gap-2 uppercase tracking-widest text-[10px]">
              ...
              {/* YAN PANEL: bg-slate-900 -> bg-card | border-slate-800 -> border-border */}
              <aside className="lg:sticky lg:top-28 space-y-6">
                <div className="bg-card p-8 rounded-[2.5rem] border border-border shadow-2xl space-y-6 transition-colors duration-300">
                  <h3 className="text-xl font-bold text-foreground tracking-tight">Akademik Durum</h3>
                  <div className="space-y-4">
                    <div className="p-5 bg-teal-500/10 rounded-2xl border border-teal-500/20">
                      ...
                      <p className="text-3xl font-black text-foreground">{earnedCertificates.length}</p>
                    </div>
            ...
            /* SERTİFİKA YOKSA: bg-slate-900/30 -> bg-muted/20 | border-slate-800 -> border-border */
            <div className="py-24 text-center bg-muted/20 rounded-[3rem] border-2 border-dashed border-border max-w-2xl mx-auto transition-colors">
              <Award className="w-16 h-16 text-muted-foreground opacity-40 mx-auto mb-6" />
              <h2 className="text-2xl font-bold text-foreground mb-2 tracking-tight">Henüz Sertifikanız Yok</h2>
              <p className="text-muted-foreground mb-8 px-8 font-medium">Üniteleri tamamlayıp quizleri çözerek ilk uzmanlık belgenizi alabilirsiniz.</p>
